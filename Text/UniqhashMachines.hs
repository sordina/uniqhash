...
changedFiles = autoT (fingerprint >>> detectChanges) ~> filterJust

fingerprint :: MealyT IO FilePath (FilePath, Maybe HASH)
fingerprint = arr id &&& hashPipe

detectChanges :: (Ord a, Eq b, Monad m) => MealyT m (a, b) (Maybe a)
detectChanges = (arr id &&& cache) >>> arr (uncurry retrieve)

-- Note: Unused in main
emitChanges :: (Ord a, Eq b) => Process (a, b) a
emitChanges = autoT detectChanges ~> filterJust
...
hashPipe :: MealyT IO FilePath (Maybe HASH)
hashPipe = arrM hash
...
cache :: (Monad m, Ord k) => MealyT m (k, v) (M.Map k v)
cache = scanMealyT (flip (uncurry M.insert)) M.empty
